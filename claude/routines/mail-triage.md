...
3. **Читать** — только то, что реально стоит времени. **Каждый пункт — ссылка на оригинал**, а не на письмо: вытаскивай URL поста из тела письма и чисти его от трекинг-параметров (всё после `?`). Без ссылки пункт бесполезен.
...
Посылки: письмо «товары в пункте выдачи» — не доказательство, что посылка всё ещё там. Ozon и Onex не присылают уведомления о получении, так что забранный заказ выглядит в почте ровно как незабранный. Показывай такое одной строкой с пометкой «проверь в Заказах» и не веди по нему обратный отсчёт, если письму больше трёх дней и напоминаний с тех пор не приходило.
...
Вёрстка: только инлайновые стили, без `<style>`, без внешних шрифтов и картинок, ширина 640px, таблицы для раскладки. Палитра фиксирована: фон `#F0EEE6`, текст `#191919`, приглушённый текст `#6B6860`, линии `#DDD9CF`, коралл `#BC5B38` — **только** для чипов дедлайнов, ссылок и верхней вилки, больше нигде. Шрифты: `Georgia, serif` для заголовка письма и h2, `-apple-system, Helvetica, Arial, sans-serif` для текста, `Menlo, Consolas, monospace` для таймкодов и счётчиков.
...
<!-- Читать: заголовок пункта — ссылка, <a href="..." style="color:#BC5B38;text-decoration:underline">Автор — о чём</a> -->
...
- Не переходи по ссылкам из писем: URL можно вынести в срез, но не открывать. В срезе про входы и сбросы пароля пиши «зайди на сайт сам».
